...
void sdp4_init( const predict_orbital_elements_t * tle, struct _sdp4 * m )
{
    ...
    double temp1, temp2, temp3, theta4, a1, a3ovk2, ao, c2, coef, coef1, x1m5th,
        xhdot1, del1, delo, eeta, eta, etasq, perigee, psisq, tsi, qoms24, s4,
        pinvsq;
    ...
    pinvsq = 1 / ( m->deep_arg.aodp * m->deep_arg.aodp * m->deep_arg.betao2 *
                   m->deep_arg.betao2 );
    ...
    c2 = coef1 * m->deep_arg.xnodp *
         ( m->deep_arg.aodp * ( 1 + 1.5 * etasq + eeta * ( 4 + etasq ) ) +
    ...
    m->c4 = 2 * m->deep_arg.xnodp * coef1 * m->deep_arg.aodp *
            m->deep_arg.betao2 *
            ( eta * ( 2 + 0.5 * etasq ) + m->eo * ( 0.5 + 2 * etasq ) -
              2 * CK2 * tsi / ( m->deep_arg.aodp * psisq ) *
                  ( -3 * m->x3thm1 *
                        ( 1 - 2 * eeta + etasq * ( 1.5 - 0.5 * eeta ) ) +
                    0.75 * m->x1mth2 * ( 2 * etasq - eeta * ( 1 + etasq ) ) *
                        cos( 2 * m->omegao ) ) );
    ...
    m->deep_arg.xmdot = m->deep_arg.xnodp +
    ...
    m->deep_arg.omgdot = -0.5 * temp1 * x1m5th +
                         0.0625 * temp2 *
                             ( 7 - 114 * m->deep_arg.theta2 + 395 * theta4 ) +
                         temp3 * ( 3 - 36 * m->deep_arg.theta2 + 49 * theta4 );
    xhdot1 = -temp1 * m->deep_arg.cosio;
    m->deep_arg.xnodot = xhdot1 +
                         ( 0.5 * temp2 * ( 4 - 19 * m->deep_arg.theta2 ) +
                           2 * temp3 * ( 3 - 7 * m->deep_arg.theta2 ) ) *
                             m->deep_arg.cosio;
                   ...
                   struct model_output * output )
{
    ...
    double a, axn, ayn, aynl, beta, betal, capu, cos2u, cosepw, cosik, cosnok,
        cosu, cosuk, ecose, elsq, epw, esine, pl, rdot, rdotk, rfdot, rfdotk,
        rk, sin2u, sinepw, sinik, sinnok, sinu, sinuk, tempe, templ, tsq, u, uk,
        ux, uy, uz, vx, vy, vz, xl, xlt, xmam, xmdf, xmx, xmy, xnoddf, xll, r,
    ...
    rk = r * ( 1 - 1.5 * temp2 * betal * m->x3thm1 ) +
    ...
    rfdotk = rfdot +
             deep_dyn.xn * temp1 * ( m->x1mth2 * cos2u + 1.5 * m->x3thm1 );
                           ...
                           deep_arg_fixed_t * deep_arg )
{
    double a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, ainv2, aqnv, sgh, sini2, sh,
        si, day, bfact, c, cc, cosq, ctem, f322, zx, zy, eoc, eq, f220, f221,
        f311, f321, f330, f441, f442, f522, f523, f542, f543, g200, g201, g211,
        s1, s2, s3, s4, s5, s6, s7, se, g300, g310, g322, g410, g422, g520,
        g521, g532, g533, gam, sinq, sl, stem, temp, temp1, x1, x2, x3, x4, x5,
        x6, x7, x8, xmao, xno2, xnodce, xnoi, xpidot, z1, z11, z12, z13, z2,
                ...
                deep_arg_dynamic_t * deep_dyn )
{
    ...
    double alfdp, sinis, sinok, sil, betdp, dalf, cosis, cosok, dbet, dls, f2,
        f3, xnoh, pgh, ph, sel, ses, xls, sinzf, sis, sll, sls, temp, x2li,
        x2omi, xl, xldot, xnddt, xndot, xomi, zf, zm, delt = 0, ft = 0;
    ...
    switch( ientry )
    {
        case DPSecular: /* Entrance for deep space secular effects */
            ...
            do
            {
                ...
                do
                {
                    ...
                    {
                        xndot = m->del1 * sin( deep_dyn->xli - m->fasx2 ) +
                                m->del2 *
                                    sin( 2 * ( deep_dyn->xli - m->fasx4 ) ) +
                                m->del3 *
                                    sin( 3 * ( deep_dyn->xli - m->fasx6 ) );
                        xnddt = m->del1 * cos( deep_dyn->xli - m->fasx2 ) +
                                2 * m->del2 *
                                    cos( 2 * ( deep_dyn->xli - m->fasx4 ) ) +
                                3 * m->del3 *
                                    cos( 3 * ( deep_dyn->xli - m->fasx6 ) );
                    }
                    ...
                    else
                    {
                        ...
                        xndot = m->d2201 * sin( x2omi + deep_dyn->xli - G22 ) +
                                m->d2211 * sin( deep_dyn->xli - G22 ) +
                                m->d3210 * sin( xomi + deep_dyn->xli - G32 ) +
                                m->d3222 * sin( -xomi + deep_dyn->xli - G32 ) +
                                m->d4410 * sin( x2omi + x2li - G44 ) +
                                m->d4422 * sin( x2li - G44 ) +
                                m->d5220 * sin( xomi + deep_dyn->xli - G52 ) +
                                m->d5232 * sin( -xomi + deep_dyn->xli - G52 ) +
                                m->d5421 * sin( xomi + x2li - G54 ) +
                                m->d5433 * sin( -xomi + x2li - G54 );
                        xnddt = m->d2201 * cos( x2omi + deep_dyn->xli - G22 ) +
                                m->d2211 * cos( deep_dyn->xli - G22 ) +
                                m->d3210 * cos( xomi + deep_dyn->xli - G32 ) +
                                m->d3222 * cos( -xomi + deep_dyn->xli - G32 ) +
                                m->d5220 * cos( xomi + deep_dyn->xli - G52 ) +
                                m->d5232 * cos( -xomi + deep_dyn->xli - G52 ) +
                                2 * ( m->d4410 * cos( x2omi + x2li - G44 ) +
                                      m->d4422 * cos( x2li - G44 ) +
                                      m->d5421 * cos( xomi + x2li - G54 ) +
                                      m->d5433 * cos( -xomi + x2li - G54 ) );
                    }
                    ...
                    {
                        deep_dyn->xli = deep_dyn->xli + xldot * delt +
                                        xndot * m->step2;
                        deep_dyn->xni = deep_dyn->xni + xndot * delt +
                                        xnddt * m->step2;
        ...
        case DPPeriodic: /* Entrance for lunar-solar periodics */
            ...
            else
            {
                ...
                xls = deep_dyn->xll + deep_dyn->omgadf +
                      cosis * deep_dyn->xnode;
                dls = deep_dyn->pl + pgh -
                      deep_dyn->pinc * deep_dyn->xnode * sinis;
                xls = xls + dls;
                xnoh = deep_dyn->xnode;
                ...
                deep_dyn->xll = deep_dyn->xll + deep_dyn->pl;
                deep_dyn->omgadf = xls - deep_dyn->xll -
                                   cos( deep_dyn->xinc ) * deep_dyn->xnode;
            }
            return;
    }
}
