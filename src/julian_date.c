...
time_t mktime_utc( const struct tm * timeinfo_utc )
{
    ...
    int timezone_diff = 0; // deviation of the current timezone from UTC in
                           // seconds
    ...
    // hack for preventing mktime from assuming localtime: add timezone
    // difference to the input struct.
...
/**
 * Helper function for getting the Julian day start date (1979-12-31 00:00 UTC)
 *as time_t.
 *
...
time_t predict_from_julian( predict_julian_date_t date )
{
    ...
    // add number of seconds since julian start day to the julian start day, get
    // current time_t
    struct tm timeinfo;
