                ...
                struct _sgp4 * m )
{
    ...
    double x1m5th, xhdot1, a1, a3ovk2, ao, betao, betao2, c1sq, c2, c3, coef,
        coef1, del1, delo, eeta, eosq, etasq, perigee, pinvsq, psisq, qoms24,
    ...
    m->c4 = 2 * m->xnodp * coef1 * m->aodp * betao2 *
            ( m->eta * ( 2 + 0.5 * etasq ) + m->eo * ( 0.5 + 2 * etasq ) -
    ...
    m->xnodot = xhdot1 + ( 0.5 * temp2 * ( 4 - 19 * theta2 ) +
                           2 * temp3 * ( 3 - 7 * theta2 ) ) *
                             m->cosio;
    ...
    m->t2cof = 1.5 * m->c1;
    m->xlcof = 0.125 * a3ovk2 * m->sinio * ( 3 + 5 * m->cosio ) /
               ( 1 + m->cosio );
    ...
    if( !m->simpleFlag )
    {
        ...
        m->d4 = 0.5 * temp * m->aodp * tsi * ( 221 * m->aodp + 31 * s4 ) *
                m->c1;
        m->t3cof = m->d2 + 2 * c1sq;
        m->t4cof = 0.25 * ( 3 * m->d3 + m->c1 * ( 12 * m->d2 + 10 * c1sq ) );
        m->t5cof = 0.2 * ( 3 * m->d4 + 12 * m->c1 * m->d3 + 6 * m->d2 * m->d2 +
                           15 * c1sq * ( 2 * m->d2 + c1sq ) );
    }
}
                   ...
                   struct model_output * output )
{
    double cosuk, sinuk, rfdotk, vx, vy, vz, ux, uy, uz, xmy, xmx, cosnok,
        ...
        sinu, cosu, betal, rfdot, rdot, r, pl, elsq, esine, ecose, epw, cosepw,
        tfour, sinepw, capu, ayn, xlt, aynl, xll, axn, xn, beta, xl, e, a,
        tcube, delm, delomg, templ, tempe, tempa, xnode, tsq, xmp, omega,
        xnoddf, omgadf, xmdf, temp, temp1, temp2, temp3, temp4, temp5, temp6;
