    ...
    double start_utc )
{
    ...
    if( predict_aos_happens( orbital_elements, observer->latitude ) &&
        !predict_is_geosynchronous( orbital_elements ) && !orbit.decayed )
    {
        // TODO: Time steps have been found in FindAOS/LOS().
        // Might be based on some pre-existing source, root-finding techniques
        // or something. Find them, and improve readability of the code and so
        // that the mathematical stability of the iteration can be checked.
        // Bisection method, Brent's algorithm? Given a coherent root finding
        // algorithm, can rather have one function for iterating the orbit and
        // then let get_next_aos/los specify bounding intervals for the root
        // finding.

        // skip the rest of the pass if the satellite is currently in range,
        // since we want the _next_ AOS.
        ...
        // fine tune the results until the elevation is within a low enough
        // threshold
 ...
 * \param direction Either POSITIVE_DIRECTION (step from current time to pass
 *end) or NEGATIVE_DIRECTION (step from current time to start of pass). In case
 *of the former, the pass will be stepped until either elevation is negative or
...
/**
 * Convenience function for calculation of derivative of elevation at specific
 *time.
 *
 * \param observer Ground station
                              ...
                              double frequency )
{
    ...
    return -frequency * sat_range_rate /
           SPEED_OF_LIGHT; // assumes that sat_range <<<<< speed of light, which
                           // is very ok
}
